...
if(isset($_POST['submit'])!=""){
  ...
  $date=$_POST['date']; 
  //$link=$_POST['link'];
$insert=mysqli_query($db,"Insert into customer(name,link,date)values('$name','$link','$date')");
if($insert){
					...
					<!--	<div class="panel-body">
						...
						<table class="table datatable-basic table-bordered table-striped table-hover">
							...
							<tbody>
								<tr>
							<?php
$select=mysqli_query($db,"select * from customer order by id desc");
...
?>                                   
									...
									<td><?php echo $date;?></td>
									<td><a href="edit_customer.php?id=<?php echo $row1['id'];?>" onclick="return confirm('You Are Going to Update Record!!')"><span class="label label-success">Edit</span></a></td>
   ...
   echo "<div style='text-align: center;font-weight:bold;left: -55px;color:red;'><h3>You dont have privilege to access this page</h3></div>";
	...
	<!-- /page container -->
<script> $(document).on('click','#delete',function(){ var element = $(this); var del_id = element.attr("data-id"); var info = 'id=' + del_id; if(confirm("Are you sure you want to delete this?")) { $.ajax({ type: "POST", url: "ajax_delete_customer.php", data: info, success: function(){  } }); $(this).parents("tr").animate({ backgroundColor: "#003" }, "slow") .animate({ opacity: "hide" }, "slow"); } return false; }); </script>
