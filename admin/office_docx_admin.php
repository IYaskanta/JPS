...
//$sdb=mysql_select_db("campusvi_share",$conn) or die(mysql_error());
		$db = mysqli_connect("localhost", "campusvi_share", "hL&RbigFG31v", "campusvi_share");
...
$insert=mysqli_query($db,"insert into office(name,file_name,date)values('$name','$file_name','$date')");
if($insert){
header("location:office_docx_admin.php");
}
else{
//die(mysql_error());
}
					...
					<!-- 2 columns form -->
						...
						<div class="panel panel-flat">
							<div class="panel-heading">
								<h5 class="panel-title">Office Docs</h5>
								<div class="heading-elements">
									<ul class="icons-list">
					...
					<!--	<div class="panel-body">
						...
						<table class="table datatable-basic table-bordered table-striped table-hover">
							...
							<tbody>
								<tr>
							<?php
$select=mysqli_query($db,"select * from office order by id desc");
while($row1=mysqli_fetch_array($select)){
	$name=$row1['name'];
	$file_name=$row1['file_name'];
	$date=$row1['date'];
?>                                   
                                    	<td><a href="#"><?php echo $file_name;?></a></td>
									<td><img src="tick.png" width="14" height="14">&nbsp;&nbsp;<a href="download.php?filename=<?php echo $name;?>"><?php echo $name ;?></a></td>
									<td><?php echo $date;?></td>
									<td><a href="office_docx_admin-edit.php?id=<?php echo $row1['id'];?>" onclick="return confirm('You Are Going to Update Record!!')"><span class="label label-success">Edit</span></a></td>
					...
					<div class="footer text-muted" style="color:black;">
					          Copyright © <?php echo date("Y"); ?> Powered by CSC.
					</div
					<!-- /footer -->
